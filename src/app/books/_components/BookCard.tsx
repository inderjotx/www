...
export function BookCard({
  ...
  return (
    <a target="_blank" href={`https://literal.club/book/${slug}`}>
      <div
        className={cn(
          "w-full hover:opacity-100 opacity-90 transition-all rounded-sm  active:ring active:ring-purple-400 bg-muted-foreground/10   flex",
          startedReading ? "h-28" : "h-24"
        )}
      >
          ...
          <div className="text-[12px]  font-sans  flex items-center  gap-2 text-purple-400 ">
            {startedReading && (
              <>
                ...
                <span>{parseDate(startedReading)}</span>
              </>
            )}
          </div>
...
export function LoadingBookCard() {
  return (
    <a target="_blank" href={`/books`}>
      <div
        className={cn(
          "w-full  active:ring active:ring-purple-400 bg-muted-foreground/10  rounded-sm flex h-20"
        )}
      >
        ...
        <div className="w-5/6 p-2 justify-center pl-5 h-full flex flex-col gap-1">
          <div className="text-[12px]  font-sans  flex items-center  gap-2 text-purple-400 ">
            <>
              <BookAudio className="size-4" /> <span>{"Fetching..."}</span>
            </>
          </div>
          <div className="font-semibold text-sm flex gap-1">
            <span className="truncate">{"Title..."}</span>
          </div>
          <div className="text-[12px] text-muted-foreground">{"Writer"}</div>
          <div></div>
        </div>
      </div>
    </a>
