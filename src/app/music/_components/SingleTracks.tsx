...
export function SingleTrack({ href, image_url, title,
    artist }: SingleTrackProps) {

    return (

        <Link href={href} >
            <div className='h-full rounded-sm overflow-hidden w-[140px] group relative inline-block' >
                <Image src={image_url} alt='image' className='object-cover' fill quality={100} sizes='100' />
                ...
                <div className='absolute transition-all top-0 z-10 h-full w-full group-hover:bg-black/80'>
                </div>
            </div>
        </Link>
    )
