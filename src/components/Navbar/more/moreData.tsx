...
import analytics from "../../../../public/navbar/more/analytics.webp";
  ...
  return (
    <div className="grid grid-cols-2 gap-2 lg:gap-1  h-full w-full">
      {data.map(({ image_url, slug }) => {
        return <ImageDisplay key={slug} slug={slug} image_url={image_url} />;
      })}
    </div>
  );
}
  ...
  return (
    <Link href={`/${slug}`} className={cn("p-1 h-16 group", className)}>
      <div className="relative flex items-center h-full  overflow-hidden rounded-sm ">
        <div className="z-20 flex font-semibold items-center gap-1 ml-2 lg:text-sm">
          {slug}{" "}
        ...
        <Image
          ...
          fill
        />
        <div className="absolute hover:bg-black/70 transition-all bg-black/60 top-0 z-10 h-full w-full "></div>
      </div>
    </Link>
  );
}
